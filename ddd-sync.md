...
Synchronize the DDD project specs with the current implementation state across all four pillars (Logic, Data, Interface, Infrastructure). This is the primary tool for resolving drift between specs and code.
...
3. **Bidirectional drift analysis** (CRITICAL — do NOT skip):
   ...
   Perform this analysis for both flows AND UI pages. For each entry where the specHash doesn't match the current spec YAML:

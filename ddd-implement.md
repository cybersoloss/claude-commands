...
11. **Implement backend flows (Logic pillar)**:
   ...
   **Follow the node graph** from trigger through all paths to terminal nodes. Each node becomes real code:
   ...
   - `ipc_call` → local IPC, native function call, or OS subprocess (e.g., Tauri invoke, Electron IPC, shell commands). Check `bridge: "shell"` for subprocess — use `child_process.execFile` or `spawn` with `working_dir`, `capture` (stdout/stderr/both), `on_nonzero_exit` (error or log_and_continue). Check `output_format` for auto-parsing: `json` → `JSON.parse(stdout)`, `plist` → plist parser, `csv` → CSV parser, `lines` → `stdout.split('\n')`, `raw` → pass as-is (default). Check `result_condition` to map return values to success/error handles (eliminates separate decision node).
   ...
   > **Advanced node fields:** The fetched DDD Usage Guide Section 6 defines the full specification for each node type. Always check the spec for type-specific fields and implement them when present. Key fields to watch for: trigger `filter` (event payload filtering — eliminates unnecessary decision nodes), trigger `debounce_ms`, trigger `event_subscriptions` (SSE triggers — generate event bus subscriptions on connection open that forward matching events as SSE messages), terminal `response_type` (json/stream/sse/empty) and `headers`, data_store `pagination` and `sort` fields, event `queue_operation` (read_metrics → call `queue.getJobCounts()`, get_jobs → call `queue.getJobs(states, start, end)` with `queue_query` filters, remove_repeatable → call `queue.removeRepeatableByKey(job_id)`).
